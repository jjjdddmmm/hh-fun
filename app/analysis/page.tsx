...
import { Home, Plus, TrendingUp, DollarSign, MapPin, Square, Bed, Bath, AlertCircle, Loader2, X, ExternalLink, Users, Trash2, RefreshCw, Hammer, PartyPopper, Handshake, Calendar, Target, Shield, Lightbulb, ChevronDown, Clock, Zap } from "lucide-react";
...
export default function PropertyAnalysisPage() {
  ...
  const forceRefreshProperty = async (property: Property) => {
    const confirmed = await confirm({
      title: "Force Refresh Property (Admin)",
      description: "This will clear all BatchData fields and re-fetch everything fresh from the API. This costs ~$0.46 per property. Continue?",
    ...
    try {
      ...
      // Call refresh endpoint with force flag to clear BatchData fields
      const refreshResponse = await fetch(`/api/properties/${property.id}/refresh`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      showSuccess("Force refresh completed - fresh data fetched from BatchData API");
    } catch (error) {
      logger.error('❌ Force refresh error:', error);
      showError('Failed to force refresh property. Please try again.', "Force Refresh Failed");
                  ...
                  <Card className="shadow-lg border-2 border-[#D9DADA] bg-[#F2F2F2] hover:shadow-xl transition-shadow overflow-hidden rounded-2xl">
                    ...
                    <div className="relative h-48 bg-gradient-to-br from-gray-100 to-gray-200">
                      ...
                      <div className="absolute top-3 right-3 flex gap-1">
                        ...
                        {/* Admin-only Force Refresh Button */}
                        {user?.emailAddresses?.[0]?.emailAddress === 'jeff@househunting.fun' && (
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => forceRefreshProperty(property)}
                            className="h-8 w-8 p-0 bg-white/20 hover:bg-yellow-500/20 backdrop-blur-sm text-white hover:text-yellow-500 transition-colors"
                            disabled={property.status === 'pending'}
                            title="Force Refresh (Admin) - Clears BatchData cache"
                          >
